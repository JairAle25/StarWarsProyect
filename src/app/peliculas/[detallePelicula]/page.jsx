    ...
    return(
        <>
            <section className="w-[90%] flex flex-col justify-center items-center mx-auto my-0 gap-5 mt-7 mb-10">
                <div className="w-4/5 text-white flex flex-col justify-center items-center gap-3 md:gap-6">
                    <h1 className="text-[var(--colorPrincipal)] text-center font-bold text-5xl md:text-6xl">{peliculaElegida.title}</h1>
                    <p className="text-xl md:text-3xl">Episodio <span className="font-bold">N° {peliculaElegida.episode_id}</span></p>
                    <p className="text-xl md:text-3xl">Dirigida por: <span className="font-bold">{peliculaElegida.director}</span></p>
                </div>
                <div className="w-1/6 md:w-1/3">
                    <Image src={portadaFilms} alt="portada pelicula star wars" className="w-full"/>
                </div>
                <div className="flex flex-col justify-center items-center w-full">
                    <h1 className="text-[var(--colorPrincipal)] text-3xl font-bold py-6 md:text-4xl">Personajes</h1>
                    <div className="w-full flex overflow-x-scroll gap-8">
                        {personajes.map((personaje) =>(
                            <Link key={personaje.name} href={`/personajes/${personaje.name.split(" ").join("-")}`} className="flex flex-col items-center justify-center min-w-[15%] mb-5 border px-2 py-3 rounded-lg transition-all duration-1000 hover:bg-[#333333] md:min-w-[30%]">
                                <h1 className="text-[var(--colorPrincipal)] text-center text-base font-semibold pb-2 md:text-2xl">{personaje.name}</h1>
